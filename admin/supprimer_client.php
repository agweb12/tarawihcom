<?php
    ...
    if(!empty($_POST))
    {
        ...
        $db = Database::connect();
        $statement = $db->prepare("DELETE FROM clients_adresses WHERE id = ?");
...
<body>
    ...
    <section id="supprimerlivreadmin" style="    display:flex;justify-content:center;justify-items: baseline;text-align: center;padding: 40px 0px 80px 0px;background-color: white;">   
        <form class="form-groupesupprimerlivre" style="max-width: 400px;height: fit-content;align-items: center;justify-content: left;margin: 10px 20px;padding: 20px;min-width: fit-content;background-color: whitesmoke;position: relative" role="form" action="supprimer_client.php" method="post">
            ...
            <h3 class="supprimerlivre" style="margin-bottom:20px;">Êtes-vous sûr de vouloir supprimer le client ?</h5>
